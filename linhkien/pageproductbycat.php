...
<?php
    if (!isset($_GET['categoryID']) || empty($_GET['categoryID'])) {
        ...
        $checkProduct = $product->get_product_by_category($_GET['categoryID']);
        if ($checkProduct) {
    ...
    <div class="container">
        <div class="page-search-inner">
            <h2 class="page-search-result-header">
                ...
                Kết quả tìm kiếm cho danh mục '<span><?php echo $checkProduct->fetch_assoc()['categoryName']; ?></span>'
            </h2>
            ...
            <ul class="page-search-items">
                <?php
                    $productByCategory = $product->get_product_by_category($categoryID, 15, $page);
            ...
            <div class="page-search-page-controller">
                <a<?php
                    if ($get_page == '' || $get_page == 1) {
                        ...
                        echo ' href="?categoryID='.$categoryID.'&page='.($get_page - 1).'"';
                    }
                ?>><i class="fas fa-chevron-left"></i></a>
                <?php
                    $pagination = $product->get_product_by_category($categoryID);
                    if ($pagination) {
                        $a = ceil($pagination->num_rows / 15);
                        for ($i = 1; $i <= $a; $i++) {
                ?>
                <a href="?categoryID=<?php echo $categoryID; ?>&page=<?php echo $i; ?>" <?php 
                    if ($get_page == '' && $i == 1) {
                        echo 'class="primary"';
                    } elseif ($i == $get_page) {
                        echo 'class="primary"';
                ...
                <a<?php
                    if ($a == 1) {
                        ...
                        echo ' href="?categoryID='.$categoryID.'&page='.($get_page + 2).'"';
                    } elseif ($get_page < $a) {
                        echo ' href="?categoryID='.$categoryID.'&page='.($get_page + 1).'"';
                    } else {
                        echo ' style="pointer-events: none;"';
