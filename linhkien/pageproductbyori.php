...
<?php
    if (!isset($_GET['originID']) || empty($_GET['originID'])) {
        ...
        $checkProduct = $product->get_product_by_origin($_GET['originID']);
        if ($checkProduct) {
    ...
    <div class="container">
        <div class="page-search-inner">
            <h2 class="page-search-result-header">
                ...
                Kết quả tìm kiếm cho xuất xứ '<span><?php echo $checkProduct->fetch_assoc()['originName']; ?></span>'
            </h2>
            ...
            <ul class="page-search-items">
                <?php
                    $productByOrigin = $product->get_product_by_origin($originID, 15, $page);
                    if ($productByOrigin) {
                        while ($result = $productByOrigin->fetch_assoc()) {
                ?>
            ...
            <div class="page-search-page-controller">
                <a<?php
                    if ($get_page == '' || $get_page == 1) {
                        ...
                        echo ' href="?originID='.$originID.'&page='.($get_page - 1).'"';
                    }
                ?>><i class="fas fa-chevron-left"></i></a>
                <?php
                    $pagination = $product->get_product_by_origin($originID);
                    if ($pagination) {
                        $a = ceil($pagination->num_rows / 15);
                        for ($i = 1; $i <= $a; $i++) {
                ?>
                <a href="?originID=<?php echo $originID; ?>&page=<?php echo $i; ?>" <?php 
                    if ($get_page == '' && $i == 1) {
                        echo 'class="primary"';
                    } elseif ($i == $get_page) {
                        echo 'class="primary"';
                ...
                <a<?php
                    if ($a == 1) {
                        ...
                        echo ' href="?originID='.$originID.'&page='.($get_page + 2).'"';
                    } elseif ($get_page < $a) {
                        echo ' href="?originID='.$originID.'&page='.($get_page + 1).'"';
                    } else {
                        echo ' style="pointer-events: none;"';
